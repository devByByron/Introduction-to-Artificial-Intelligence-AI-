...
Example: Chess-playing AIs like IBM’s Deep Blue that make moves based on current board setup, without learning from past games.
...
Example: A robot that knows it’s a robot, has its own thoughts, and can make independent decisions—like a character from a sci-fi movie (e.g., Ava from Ex Machina).
